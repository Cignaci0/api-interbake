...
let MarcasService = class MarcasService {
    ...
    configService;
    constructor(marcasRepository, mailerService, marcasAuditoriaRepository, configService) {
    ...
    async findAll(numFicha, fechaInicio, fechaFin) {
        const busqueda = await this.marcasRepository.find({
            where: {
                num_ficha: numFicha,
                fecha_marca: (0, typeorm_2.Between)(fechaInicio, fechaFin),
            },
            order: {
                fecha_marca: 'ASC',
            },
        ...
        if (busqueda.length > 0 && busqueda[0].empleado) {
            ...
            empleadoInfo = await this.marcasRepository.manager.findOne(empleado_entity_1.Empleado, {
                where: { num_ficha: numFicha },
                relations: ['turno', 'turno.detalle_turno', 'turno.detalle_turno.horario_id']
            });
        }
        if (!empleadoInfo) {
            throw new common_1.HttpException('No se pudo encontrar el empleado', 404);
        }
        const diasTurnoQuery = await this.marcasRepository.manager.query(`SELECT e.turno_id, dt.cod_dia 
       FROM empleado e 
       LEFT JOIN detalle_turno dt ON dt.turno_id = e.turno_id 
       WHERE e.num_ficha = $1`, [numFicha]);
        let diasConTurno = [1, 2, 3, 4, 5, 6, 7];
        if (diasTurnoQuery && diasTurnoQuery.length > 0) {
            if (diasTurnoQuery[0].turno_id !== null) {
                diasConTurno = diasTurnoQuery.filter((row) => row.cod_dia !== null).map((row) => Number(row.cod_dia));
            }
        }
        ...
        while (currentDate <= endDate) {
            ...
            const marcasDelDia = busqueda.filter((m) => {
                ...
                if (typeof m.fecha_marca === 'string') {
                    mDateKey = m.fecha_marca.substring(0, 10);
                }
            ...
            if (marcasDelDia.length > 0) {
                const formateadas = marcasDelDia.map(m => {
                    let dtDia = empleadoInfo?.turno?.detalle_turno?.find((dt) => Number(dt.cod_dia) === Number(diaSemana));
                    let overrideHorario = dtDia && dtDia.horario_id ? dtDia.horario_id : null;
                    return {
                        ...m,
                        tieneTurno: tieneTurnoHoy,
                        fecha_marca: fechaFormatExt,
    ...
    async update(id, updateMarcaDto, usuarioActualizador = 'Sistema') {
        if (!updateMarcaDto || Object.keys(updateMarcaDto).length === 0) {
            throw new common_1.HttpException('No se proporcionaron los datos para actualizar la marca', 400);
        }
        const marcaOriginal = await this.marcasRepository.findOne({ where: { marca_id: id } });
        if (!marcaOriginal) {
            throw new common_1.HttpException('No se encontró la marca a actualizar', 404);
        }
        ...
        if (fechaPropuesta instanceof Date) {
            fechaPropuesta = fechaPropuesta.toISOString().substring(0, 10);
        }
        else if (typeof fechaPropuesta === 'string') {
            fechaPropuesta = fechaPropuesta.substring(0, 10);
        }
        ...
        if (marcaAuditoria.fecha_actualizacion) {
            if (marcaAuditoria.fecha_actualizacion instanceof Date) {
                marcaAuditoria.fecha_actualizacion = marcaAuditoria.fecha_actualizacion.toISOString().substring(0, 19).replace('T', ' ');
            }
        }
        ...
        try {
            ...
            if (empleadoInfo && empleadoInfo.email) {
                ...
                let eventoNombre = 'Marca';
                if (String(marcaAuditoria.evento) === '1')
                    eventoNombre = 'Entrada';
                if (String(marcaAuditoria.evento) === '2')
                    eventoNombre = 'Salida';
                ...
                const direccion = 'ojos del salado 0851';
                const comuna = 'QUILICURA';
                const urlBase = this.configService.get('API_URL_BASE') || 'http://localhost:3000';
    ...
    async confirmarCambio(token, accion) {
    ...
    async remove(id) {
        ...
        if (!marca) {
            throw new common_1.HttpException('No se encontró la marca a eliminar', 404);
        }
        const empleadoInfo = await this.marcasRepository.manager.findOne(empleado_entity_1.Empleado, {
        ...
        if (empleadoInfo && empleadoInfo.email) {
            ...
            const nombreEmpleado = empleadoInfo.nombres + ' ' + empleadoInfo.apellido_paterno + ' ' + empleadoInfo.apellido_materno;
            let eventoNombre = 'Marca';
            if (String(marca.evento) === '1')
                eventoNombre = 'Entrada';
            if (String(marca.evento) === '2')
                eventoNombre = 'Salida';
        ...
        await this.marcasAuditoriaRepository.delete({ marca: { marca_id: id } });
    ...
    async procesarAprobacionesAutomaticas() {
        ...
        const pendientes = await this.marcasAuditoriaRepository.find({
            where: {
                estado: 3,
                fecha_actualizacion: (0, typeorm_2.Between)(new Date(0), hace48Horas)
            }
        });
...
], MarcasService.prototype, "procesarAprobacionesAutomaticas", null);
exports.MarcasService = MarcasService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(marca_entity_1.Marca)),
    __param(2, (0, typeorm_1.InjectRepository)(marcas_auditoria_entity_1.MarcasAuditoria)),
